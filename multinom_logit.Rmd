...
occupancy_data <- occupancy_data_with_lines %>%
  mutate(
...
weather <- st_read("./data/weather_data_with_city_coordinates.geojson") %>%
  st_drop_geometry() %>%
  dplyr::select(-station_id) 

  weather$time <- as.character(str_pad(weather$time, width = 4, pad = "0"))  # Pad to 4 digits


weather_sf <- st_as_sf(weather, coords= c("city_long", "city_lat"), crs = crs(stations_sf)) %>%
  mutate( geo_f = as.factor(geometry))

trips_to_station <- trips_to_station %>%
  mutate(rounded_timestamp = round_date(timestamp, "3 hours")) %>%
  st_as_sf()

weather_stations <- st_join(
  trips_to_station,
  weather_sf,
  join = st_nearest_feature
)

weather_stations$weather_timestamp <- as.POSIXct(
  paste(weather_stations$date.y, sub("(\\d{2})(\\d{2})", "\\1:\\2", weather_stations$time)),
  format = "%Y-%m-%d %H:%M"
)

result <- weather_stations %>%
  filter(rounded_timestamp == weather_timestamp) 
...
fit_multinom <- multinom(occupancy_num ~ hour_f + date + humidity + vehicle_type + weekday_f + MS_MEDIAN_imputed.to + temperature + occupancy_lag1+ MS_MEDIAN_imputed.from +pop_dens.from + pop_dens.to + spatial_lag, data = data.Train)
